	...
	return (
		<Stack bgcolor="white" p={5} spacing={2}>
			...
			<Grid container columns={13} rowSpacing={2}>
				<Grid item xs={6}>
					<Typography pb={1} color="#424242" fontSize="0.95rem">
						Hãng xe
					</Typography>
					<Select fullWidth displayEmpty value={brand} onChange={(event) => setBrand(event.target.value)}>
						<MenuItem value="NONE">Chọn hãng xe</MenuItem>
						{brandlist.map((item) => (
				...
				<Grid item xs={6}>
					...
					>
						...
						{modellist.map((item) => (
							<MenuItem value={item.model} key={item.id}>
								{item.model}
							</MenuItem>
						))}
						<MenuItem value="KHÁC">KHÁC</MenuItem>
				...
				<Grid item xs={6}>
					...
					<Select fullWidth displayEmpty value={type} onChange={(event) => setType(event.target.value)}>
				...
				<Grid item xs={6}>
					...
					<Select fullWidth displayEmpty value={transmission} onChange={(event) => setTransmission(event.target.value)}>
				...
				<Grid item xs={6}>
					...
					<TextField fullWidth value={year} onChange={(event) => setYear(event.target.value)} placeholder="2021"></TextField>
				</Grid>
			</Grid>
			<Typography fontWeight="bold" fontSize="1.2rem">
